...
@if ($institution->reception->phone || $institution->reception->phone_2)
<div class="flex items-center mb-3">
    <div class="flex-none mr-2 lg:mr-3">
    ...
    @if ($institution->reception->phone)
        <a href="tel:{{ $institution->reception->phone }}" class="text-green-light lg:text-lg lg:text-white lg:no-underline">
            {{ $institution->reception->phone }} {{ $institution->reception->phone_2 ? ', ' : '' }} &nbsp;
        </a>
    @endif

    @if ($institution->reception->phone_2)
        <a href="tel:{{ $institution->reception->phone_2 }}" class="text-green-light lg:text-lg lg:text-white lg:no-underline">
            {{ $institution->reception->phone_2 }}
        </a>
    @endif
</div>
@endif
